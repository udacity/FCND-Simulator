var LibraryWebSockets = {
  ...
  SocketCreate: function(url) {
    ...
    socket.socket.onmessage = function(e) {
      ...
      if (socket.messages.length > 0 && socket.messages.length % 300 == 0)
        console.log("Messages are piling up! Please dequeue messages!");
    };
  ...
  SocketRecv: function(socketInstance, ptr, length) {
    ...
    if (msg.length > length) msg = msg.slice(0, length);
    HEAPU8.set(msg, ptr);
