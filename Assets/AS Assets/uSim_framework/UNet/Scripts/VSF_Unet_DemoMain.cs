...
public class VSF_Unet_DemoMain :  MonoBehaviour{


	public GameObject mainCamera;
	public Camera mapCamera;
	public GUISkin mapSkin;
	public GameObject[] vehiclesPool;
	...
	public VsfPlayerEntity playerEntity;

	public static VSF_Unet_DemoMain main;
	...
	IEnumerator Start () {
		...
		GameObject.FindObjectOfType<UnityEngine.Networking.NetworkManagerHUD> ().showGUI = false;
	}
	...
	public void SetSelectedVehicle (int index){
		...
		selectedIndex = index;
		if (player != null ) 
			playerEntity.CmdDestroyPlayer (main.player);
	
			playerEntity.SpawnVehicle(playerEntity.gameObject);				
	...
	public void SetStartFlying (bool set){

		startFlying = set;

	}

	public void HideLocationsIcons (){

		foreach (SpawnBaseData data in locations) {
	...
	public void FillVehicles (int ownerId){

		vehiclesDropdown.ClearOptions ();
		availableVehicles = new List<GameObject> ();

		List<string> options = new List<string>();
		foreach (GameObject veh in vehiclesPool) {

			UsimVehicle vehicle = veh.GetComponent<UsimVehicle> ();
			if (vehicle.GetComponent<AiRadar> ().ownerId == ownerId) {
				options.Add (vehicle.name + " | " + vehicle.description);
				availableVehicles.Add (vehicle.gameObject);
			}
		}
	...
	public void SetSpawnPoint (int index) {
		...
		currentSpawnPoint = availableSpawns [index].transform;
		if (player != null) {
			MovePlayer ();
		}
	}
	...
	public GameObject SpawnSelectedVehicle (){
	...
	IEnumerator SetFlight (){
		...
		if (startFlying) {
			...
			player.GetComponent<Rigidbody> ().velocity = player.transform.forward * 80f;
