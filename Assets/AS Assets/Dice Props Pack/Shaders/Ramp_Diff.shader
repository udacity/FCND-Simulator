...
////// Lighting:
                ...
                float2 node_393 = i.uv0;
                float node_216 = _Bands;
                float3 node_41 = normalDirection;
                float3 finalColor = (attenuation*_LightColor0.rgb*(tex2D(_Diffuse,TRANSFORM_TEX(node_393.rg, _Diffuse)).rgb+UNITY_LIGHTMODEL_AMBIENT.rgb+(round((node_216*pow(max(0,dot(node_41,halfDirection)),exp2(((_Gloss*10.0)+1.0)))))/node_216)));
/// Final Color:
...
////// Lighting:
                ...
                float node_216 = _Bands;
                float3 node_41 = normalDirection;
                float3 finalColor = (attenuation*_LightColor0.rgb*(tex2D(_Diffuse,TRANSFORM_TEX(node_394.rg, _Diffuse)).rgb+UNITY_LIGHTMODEL_AMBIENT.rgb+(round((node_216*pow(max(0,dot(node_41,halfDirection)),exp2(((_Gloss*10.0)+1.0)))))/node_216)));
/// Final Color:
